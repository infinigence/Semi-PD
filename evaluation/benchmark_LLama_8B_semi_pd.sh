...
FILE_NAME=LLama_8B_semi_pd.log
...
nohup python -m sglang.launch_server   --model-path  $MODEL_PATH --trust-remote-code \
   --context-length 10240 --watchdog-timeout 60000 --dist-timeout 3600   --enable-metrics  --disable-radix-cache  --host 0.0.0.0 \
      --served-model-name  deepseek --mem-fraction-static 0.82  --tp 1 --enable-semi-pd > $PWD/$FILE_NAME 2>&1 &
...
--model  $MODEL_PATH  --dataset-path  $DATASET_PATH  --num-prompt 1000  --benchmark-save-path $PWD/result_LLama_8B_semi_pd --request-rate 5 --request-rate-extent 10
