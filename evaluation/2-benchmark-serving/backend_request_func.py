        ...
        try:
            async with session.post(url=api_url, json=payload) as response:
                if response.status == 200:
                    ...
                    try:
                        # to_print = json.loads(body)["token"]["count_output_tokens"]
                        # output.output_len = json.loads(body)["token"]["count_output_tokens"]
                        # output.success = True
                        # print(json.loads(body))
                        # for DistServe
                        output.generated_text = json.loads(body)["text"]
                        # print(output.generated_text)
                        # output.generated_text = json.loads(body)["text"][0][len(request_func_input.prompt):]
                        output.success = True
