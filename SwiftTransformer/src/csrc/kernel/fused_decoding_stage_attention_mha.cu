...
template<
	...
	int64_t BLOCK_SIZE,
	int64_t THREAD_BLOCK_SIZE
> __global__ void fusedDecodingStageAttentionGQAKernel(
	// The output
	T* __restrict__ result,			// [num_tokens, num_heads, head_dim]
	...
	const int head_id = blockIdx.x;		// Grid: num_heads x num_decoding_reqs
	const int num_heads = gridDim.x;	// TODO Pass it as a template parameter to save a register
	const int head_kv_id = (num_heads / num_kv_heads < 6) ? head_id >> 2 : head_id >> 3;   // we only support Llama3-8B (32, 8) and Llama3-70B (64, 8)
	// const int num_kv_heads = 8;  
	const int kv_id_per_q = (num_heads / num_kv_heads < 6) ? head_id & 3 : head_id & 7; 

	const int64_t req_index = ith_decoding_req_req_index[blockIdx.y];
	const int64_t token_index = ith_decoding_req_token_index[blockIdx.y];
	const int64_t input_len = input_lens[req_index];	// Here input_lens DOES NOT INCLUDE the latest token!
	const int num_blocks = (input_len+1 + BLOCK_SIZE - 1) / BLOCK_SIZE;

	const int warp_id = threadIdx.x / WARP_SIZE;	// Which warp we are in
	const int lane_id = threadIdx.x % WARP_SIZE;
	const int thread_group_id = lane_id / NUM_THREAD_PER_KEY;	// Which thread group we are in, i.e. which column in K^T we are responsible for
	const int thread_id_in_group = lane_id % NUM_THREAD_PER_KEY;	// Which thread we are in the thread group
	...
	// Step 0: Save the KV cache
	// Only the first KV head corresponding to the same Q needs to act
	if ((threadIdx.x < HEAD_DIM) && (kv_id_per_q == 0)) {
		int64_t kvcache_index = INDEX_5D(
			0, num_layers, num_kv_heads, BLOCK_SIZE, HEAD_DIM,
			block_table[INDEX_2D(0, max_num_block_per_seq, req_index, input_len/BLOCK_SIZE)],
			0, head_kv_id, input_len%BLOCK_SIZE, threadIdx.x
		);
		k_cache_offseted[kvcache_index] = qkvs[INDEX_3D(0, num_heads+2*num_kv_heads, HEAD_DIM, token_index, num_heads+head_kv_id, threadIdx.x)];
		v_cache_offseted[kvcache_index] = qkvs[INDEX_3D(0, num_heads+2*num_kv_heads, HEAD_DIM, token_index, num_heads+num_kv_heads+head_kv_id, threadIdx.x)];
	}
	__syncthreads();	// Since we are going to use k_cache and v_cache later
	...
	for (int64_t i = 0; i < NUM_ELEM_PER_THREAD; i++) {
		// q_cache[i] = ((const T2 *)qkvs)[INDEX_4D(0, 3, num_heads, HEAD_DIM/2, token_index, 0, head_id, thread_id_in_group + i*THREAD_GROUP_SIZE)];
		q_cache[i] = ((const T2 *)qkvs)[INDEX_3D(0, num_heads+2*num_kv_heads, HEAD_DIM/2, token_index, head_id, thread_id_in_group + i*THREAD_GROUP_SIZE)];
	}
	...
	for (int64_t block_idx = warp_id; block_idx < num_blocks; block_idx += NUM_WARPS) {
		const int64_t block_index = block_table[req_index*max_num_block_per_seq + block_idx];
		const T2* k_block = (const T2*)(k_cache_offseted + (block_index*num_layers*num_kv_heads + head_kv_id)*BLOCK_SIZE*HEAD_DIM);
		const int64_t token_idx = block_idx*BLOCK_SIZE + thread_group_id;
	...
	for (int block_idx = warp_id; block_idx < num_blocks; block_idx += NUM_WARPS) {
		const int64_t block_index = block_table[req_index*max_num_block_per_seq + block_idx];
		const T2* v_block = (const T2*)(v_cache_offseted + (block_index*num_layers*num_kv_heads + head_kv_id)*BLOCK_SIZE*HEAD_DIM);
		const int64_t token_idx = block_idx*BLOCK_SIZE;
...
#define LAUNCH_DECODING_STAGE_ATTENTION_MHA_KERNEL(T, HEAD_DIM, BLOCK_SIZE, stream) \
	fusedDecodingStageAttentionMHAKernel<T, HEAD_DIM, BLOCK_SIZE, DEFAULT_THREAD_BLOCK_SIZE><<<grid_dim, DEFAULT_THREAD_BLOCK_SIZE, shared_mem_size, stream>>>( \
...
#define FUSED_DECODING_STAGE_ATTENTION_MHA_DISPATCH_HEAD_DIM_BLOCK_SIZE(T, HEAD_DIM, stream) \
	switch (block_size) { \
		case 1: LAUNCH_DECODING_STAGE_ATTENTION_MHA_KERNEL(T, HEAD_DIM, 1, stream); break; \
		case 2: LAUNCH_DECODING_STAGE_ATTENTION_MHA_KERNEL(T, HEAD_DIM, 2, stream); break; \
		case 4: LAUNCH_DECODING_STAGE_ATTENTION_MHA_KERNEL(T, HEAD_DIM, 4, stream); break; \
		case 8: LAUNCH_DECODING_STAGE_ATTENTION_MHA_KERNEL(T, HEAD_DIM, 8, stream); break; \
		case 16: LAUNCH_DECODING_STAGE_ATTENTION_MHA_KERNEL(T, HEAD_DIM, 16, stream); break; \
		case 32: LAUNCH_DECODING_STAGE_ATTENTION_MHA_KERNEL(T, HEAD_DIM, 32, stream); break; \
		default: fprintf(stderr, "Unsupported block_size: %ld\n", block_size); assert(0); \
	}

#define FUSED_DECODING_STAGE_ATTENTION_MHA_DISPATCH_HEAD_DIM(T, stream) \
	switch (head_dim) {	\
		case 64: FUSED_DECODING_STAGE_ATTENTION_MHA_DISPATCH_HEAD_DIM_BLOCK_SIZE(T, 64, stream); break;	\
		case 80: FUSED_DECODING_STAGE_ATTENTION_MHA_DISPATCH_HEAD_DIM_BLOCK_SIZE(T, 80, stream); break;	\
		case 128: FUSED_DECODING_STAGE_ATTENTION_MHA_DISPATCH_HEAD_DIM_BLOCK_SIZE(T, 128, stream); break;	\
...
void fusedDecodingStageAttentionMHA(
	...
	FUSED_DECODING_STAGE_ATTENTION_MHA_DISPATCH_HEAD_DIM(T, stream);
}


#define LAUNCH_DECODING_STAGE_ATTENTION_GQA_KERNEL(T, HEAD_DIM, BLOCK_SIZE, stream) \
	fusedDecodingStageAttentionGQAKernel<T, HEAD_DIM, BLOCK_SIZE, DEFAULT_THREAD_BLOCK_SIZE><<<grid_dim, DEFAULT_THREAD_BLOCK_SIZE, shared_mem_size, stream>>>( \
...
#define FUSED_DECODING_STAGE_ATTENTION_GQA_DISPATCH_HEAD_DIM_BLOCK_SIZE(T, HEAD_DIM, stream) \
	switch (block_size) { \
		case 1: LAUNCH_DECODING_STAGE_ATTENTION_GQA_KERNEL(T, HEAD_DIM, 1, stream); break; \
		case 2: LAUNCH_DECODING_STAGE_ATTENTION_GQA_KERNEL(T, HEAD_DIM, 2, stream); break; \
		case 4: LAUNCH_DECODING_STAGE_ATTENTION_GQA_KERNEL(T, HEAD_DIM, 4, stream); break; \
		case 8: LAUNCH_DECODING_STAGE_ATTENTION_GQA_KERNEL(T, HEAD_DIM, 8, stream); break; \
		case 16: LAUNCH_DECODING_STAGE_ATTENTION_GQA_KERNEL(T, HEAD_DIM, 16, stream); break; \
		case 32: LAUNCH_DECODING_STAGE_ATTENTION_GQA_KERNEL(T, HEAD_DIM, 32, stream); break; \
		default: fprintf(stderr, "Unsupported block_size: %ld\n", block_size); assert(0); \
	}

#define FUSED_DECODING_STAGE_ATTENTION_GQA_DISPATCH_HEAD_DIM(T, stream) \
	switch (head_dim) {	\
		case 64: FUSED_DECODING_STAGE_ATTENTION_GQA_DISPATCH_HEAD_DIM_BLOCK_SIZE(T, 64, stream); break;	\
		case 80: FUSED_DECODING_STAGE_ATTENTION_GQA_DISPATCH_HEAD_DIM_BLOCK_SIZE(T, 80, stream); break;	\
		case 128: FUSED_DECODING_STAGE_ATTENTION_GQA_DISPATCH_HEAD_DIM_BLOCK_SIZE(T, 128, stream); break;	\
		default: fprintf(stderr, "Unsupported head_dim: %ld\n", head_dim); assert(0);			\
	}

template<typename T>
void fusedDecodingStageAttentionGQA(
	T* __restrict__ result,
	const T* __restrict__ qkvs,
	T* k_cache,
	T* v_cache,
	...
	const int64_t NUM_THREAD_PER_KEY = WARP_SIZE / block_size;	// The size of the thread group
	assert_whenever((head_dim/2) % NUM_THREAD_PER_KEY == 0);
	T* k_cache_offseted = k_cache + layer_id * num_kv_heads * block_size * head_dim; // num_kv_heads = 8
	T* v_cache_offseted = v_cache + layer_id * num_kv_heads * block_size * head_dim;
	dim3 grid_dim(num_q_heads, num_decoding_reqs);
	int shared_mem_size = std::max(((max_decoding_req_len+1 + block_size-1) / block_size) * block_size * sizeof(float), DEFAULT_THREAD_BLOCK_SIZE*2*sizeof(T));
	FUSED_DECODING_STAGE_ATTENTION_GQA_DISPATCH_HEAD_DIM(T, stream);
}
...
template void fusedDecodingStageAttentionGQA(
	half* __restrict__ result,
	const half* __restrict__ qkvs,
	half* k_cache,
	half* v_cache,
