...
void addbias(
	...
	addbiasKernel<T><<<gridSize, blockSize, 0, stream>>>(output, input, bias, size);
}

template void addbias(half* output, const half* input, const half* bias, const int64_t size, cudaStream_t stream);
template void addbias(float* output, const float* input, const float* bias, const int64_t size, cudaStream_t stream);
...
void addbiasBatched(
	...
	addbiasBatchedKernel<T><<<gridSize, blockSize, 0, stream>>>(output, input, bias, batch, size);
}

template void addbiasBatched(half* output, const half* input, const half* bias, const int64_t batch, const int64_t size, cudaStream_t stream);
template void addbiasBatched(float* output, const float* input, const float* bias, const int64_t batch, const int64_t size, cudaStream_t stream);
