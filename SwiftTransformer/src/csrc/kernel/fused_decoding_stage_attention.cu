...
#define LAUNCH_DECODING_STAGE_ATTENTION_KERNEL(T, Q_HEADS_PER_THREAD_BLOCK, HEAD_DIM, BLOCK_SIZE, stream) \
	fusedDecodingStageAttentionKernel<T, Q_HEADS_PER_THREAD_BLOCK, HEAD_DIM, BLOCK_SIZE, DEFAULT_THREAD_BLOCK_SIZE><<<grid_dim, DEFAULT_THREAD_BLOCK_SIZE, 0, stream>>>( \
...
#define FUSED_DECODING_STAGE_ATTENTION_DISPATCH_BLOCK_SIZE(T, Q_HEADS_PER_THREAD_BLOCK, HEAD_DIM, stream) \
	switch (block_size) { \
		case 1: LAUNCH_DECODING_STAGE_ATTENTION_KERNEL(T, Q_HEADS_PER_THREAD_BLOCK, HEAD_DIM, 1, stream); break; \
		case 2: LAUNCH_DECODING_STAGE_ATTENTION_KERNEL(T, Q_HEADS_PER_THREAD_BLOCK, HEAD_DIM, 2, stream); break; \
		case 4: LAUNCH_DECODING_STAGE_ATTENTION_KERNEL(T, Q_HEADS_PER_THREAD_BLOCK, HEAD_DIM, 4, stream); break; \
		case 8: LAUNCH_DECODING_STAGE_ATTENTION_KERNEL(T, Q_HEADS_PER_THREAD_BLOCK, HEAD_DIM, 8, stream); break; \
		case 16: LAUNCH_DECODING_STAGE_ATTENTION_KERNEL(T, Q_HEADS_PER_THREAD_BLOCK, HEAD_DIM, 16, stream); break; \
		case 32: LAUNCH_DECODING_STAGE_ATTENTION_KERNEL(T, Q_HEADS_PER_THREAD_BLOCK, HEAD_DIM, 32, stream); break; \
		default: fprintf(stderr, "Unsupported block_size: %ld\n", block_size); assert(0); \
	}

#define FUSED_DECODING_STAGE_ATTENTION_DISPATCH_HEAD_DIM(T, Q_HEADS_PER_THREAD_BLOCK, stream) \
	switch (head_dim) {	\
		case 64: FUSED_DECODING_STAGE_ATTENTION_DISPATCH_BLOCK_SIZE(T, Q_HEADS_PER_THREAD_BLOCK, 64, stream); break;	\
		case 80: FUSED_DECODING_STAGE_ATTENTION_DISPATCH_BLOCK_SIZE(T, Q_HEADS_PER_THREAD_BLOCK, 80, stream); break;	\
		case 128: FUSED_DECODING_STAGE_ATTENTION_DISPATCH_BLOCK_SIZE(T, Q_HEADS_PER_THREAD_BLOCK, 128, stream); break;	\
		default: fprintf(stderr, "Unsupported head_dim: %ld\n", head_dim); assert(0);			\
	}

#define FUSED_DECODING_STAGE_ATTENTION_DISPATCH_Q_HEADS_PER_THREAD_BLOCK(T, stream) \
	switch (q_heads_per_thread_block) {	\
		case 1: FUSED_DECODING_STAGE_ATTENTION_DISPATCH_HEAD_DIM(T, 1, stream); break;	\
		case 2: FUSED_DECODING_STAGE_ATTENTION_DISPATCH_HEAD_DIM(T, 2, stream); break;	\
...
void fusedDecodingStageAttention(
	...
	// FUSED_DECODING_STAGE_ATTENTION_DISPATCH_Q_HEADS_PER_THREAD_BLOCK(T, stream);
	fusedDecodingStageAttentionGQA(
		result,
		qkvs,
		k_cache,
		v_cache,
