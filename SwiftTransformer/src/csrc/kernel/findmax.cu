...
void findmaxBatched(
	...
	findmaxBatchedKernel<T><<<batch_size, threads, 0, stream>>>(max_indices, input, batch_size, length);
}

template void findmaxBatched(int64_t* max_indices, const half* input, const int64_t batch_size, const int64_t length, cudaStream_t stream);
template void findmaxBatched(int64_t* max_indices, const float* input, const int64_t batch_size, const int64_t length, cudaStream_t stream);
