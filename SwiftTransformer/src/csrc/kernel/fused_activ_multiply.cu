...
__global__ void fusedActivationMultiplyKernel(
	...
	for (int64_t i = blockIdx.x * blockDim.x + threadIdx.x; i < n/2; i += blockDim.x * gridDim.x) {
		...
		int64_t row = i / (ld / 2);
		int64_t col = i % (ld / 2);
		T2 input1_elem = ((const T2*)input2)[row * ld + col];
		T2 input2_elem = ((const T2*)input2)[row * ld + col + ld / 2];
		T2 output_elem = {
...
void fusedActivationMultiply(
	...
	switch (activation_type) {
		case ActivationType::RELU:
			fusedActivationMultiplyKernel<T, ActivationType::RELU><<<gridSize, blockSize, 0, stream>>>(output, input1, input2, ld, n);
			return;
		case ActivationType::SILU:
			fusedActivationMultiplyKernel<T, ActivationType::SILU><<<gridSize, blockSize, 0, stream>>>(output, input1, input2, ld, n);
			return;
		case ActivationType::GELU:
			fusedActivationMultiplyKernel<T, ActivationType::GELU><<<gridSize, blockSize, 0, stream>>>(output, input1, input2, ld, n);
			return;
		default:
			assert (false);
	}
