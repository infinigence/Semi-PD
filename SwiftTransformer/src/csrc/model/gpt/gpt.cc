		...
		)
{
	...
	// CUDA_CHECK(cudaStreamCreate(&(this->compute_stream_)));
	// this->cublas_wrapper.SetStream(this->compute_stream_);
}
...
template<typename T>
void Gpt<T>::init_p2p_communicator(const ncclUniqueId& p2p_id){
	int64_t p2p_rank = parallelism_param.is_context ? 0 : 1;
	cudaStream_t stream;
	cudaStreamCreate(&stream);
	p2p_comm = util::stNcclInit(
		2,
		p2p_rank,
		p2p_id,
		stream,
...
template<typename T>
void Gpt<T>::send_weight(int64_t layer_id){
	util::stNcclSend(this->weight.layer_weights[layer_id].fsdp_weight, this->weight.fsdp_flat_param_size_, st::util::stNcclGetDataType<T>(), 1, p2p_comm, p2p_comm.stream);
}

template<typename T>
void Gpt<T>::recv_weight(int64_t layer_id){
	util::stNcclRecv(this->weight.fsdp_layer_weight.fsdp_weight, this->weight.fsdp_flat_param_size_, st::util::stNcclGetDataType<T>(), 0, p2p_comm, p2p_comm.stream);
}

template<typename T>
void Gpt<T>::wait_stream(){
}

template<typename T>
void Gpt<T>::nccl_group_start(){
	util::stNcclGroupStart();
}


template<typename T>
void Gpt<T>::nccl_group_end(){
	util::stNcclGroupEnd();
}
...
template<typename T>
void Gpt<T>::initDummyWeightFSDP() {
	this->weight.initDummyWeightFSDP();
}

template<typename T>
std::vector<torch::Tensor>  Gpt<T>::GetFlattenWeight() {
	return this->weight.GetFlattenWeight();
}

template<typename T>
void Gpt<T>::LazyInitWeight(bool enable_ipc_mem, bool is_alloc) {
	this->weight.LazyInitWeight(enable_ipc_mem, is_alloc);
}
...
void Gpt<T>::InitDecoder() {
	...
	for (int64_t i = 0; i < batch_size; ++i) {
		num_tokens += h_is_context_stage[i] ? h_input_lens[i] : 1;
	}
	...
	num_context_reqs = 0, num_decoding_reqs = 0;
	...
	max_context_req_len = 0; 
	max_decoding_req_len = 0;
	for (int64_t i = 0, cur_token_index = 0; i < batch_size; ++i) {
		if (h_is_context_stage[i]) {
			...
			max_context_req_len = std::max(max_context_req_len, h_input_lens[i]);
			num_context_reqs += 1;
			cur_token_index += h_input_lens[i];
		} else {
			ith_decoding_req_req_index_cpu[num_decoding_reqs] = i;
			ith_decoding_req_token_index_cpu[num_decoding_reqs] = cur_token_index;
			max_decoding_req_len = std::max(max_decoding_req_len, h_input_lens[i]);
			num_decoding_reqs += 1;
			cur_token_index += 1;
		}
	}
...
template<typename T>
void Gpt<T>::ForwardBlock(int64_t layer_id) {
		T* d_output = d_decoder_output.ptr;					// [num_tokens, hidden_size]
		const T* d_input = d_decoder_input.ptr;
		...
		nvtxRangePush("Layernorm / RMSNorm");
		...
		//   attention_out: [num_tokens, hidden_size]
		//	 k/v cache
		nvtxRangePush("Self-attention");
		st::layer::attention<T>(
			attention_out.ptr,
			static_cast<T*>(d_k_cache),
			static_cast<T*>(d_v_cache),
			...
			// fsdp
			// this->parallelism_param.is_context ? cur_layer_weight.attn_qkv_kernel : weight.fsdp_layer_weight.attn_qkv_kernel,
			...
			// fsdp
			// this->parallelism_param.is_context ? cur_layer_weight.attn_out_kernel : weight.fsdp_layer_weight.attn_out_kernel,
			...
			hyper_param.is_rotary_posi_embedding_scaling,
			layer_id,
		...
		nvtxRangePush("AddResidual & LayerNorm");
		if (hyper_param.is_rmsnorm) {
			st::kernel::addbias<T>(
				attention_out.ptr,
				attention_out.ptr,
		...
		nvtxRangePop();
		nvtxRangePush("Ffn (TODO Gated FFN)");
		...
		if (hyper_param.is_gated_ffn) {
			st::layer::gatedFfn<T>(
				...
				// fsdp
				// this->parallelism_param.is_context ? cur_layer_weight.ffn_fc1_weight : weight.fsdp_layer_weight.ffn_fc1_weight,
				// this->parallelism_param.is_context ? cur_layer_weight.ffn_fc2_weight : weight.fsdp_layer_weight.ffn_fc2_weight,
				// this->parallelism_param.is_context ? cur_layer_weight.ffn_fc3_weight : weight.fsdp_layer_weight.ffn_fc3_weight,
...
template<typename T>
torch::Tensor Gpt<T>::GetCosSinCache(){
	return this->weight.get_cos_sin_cache();
}
...
void Gpt<T>::forwardDecoder(
	...
	int32_t* d_block_table,					// [batch_size, max_num_block_per_seq]
	const int64_t block_num_per_layer,      // [hongke@1014]
	const int32_t* d_input_len,				// [batch_size], gpu. The length of the i-th input. For context stage it is the number of tokens in user's request, while for decoder stage it is the number of previous tokens 

	const int32_t* h_input_len,			// [batch_size], cpu
	...
	attn_out_buf.remalloc(num_tokens * local_q_head_num * hyper_param.head_dim);
	ffn_inter_buf_1.remalloc(num_tokens * local_ffn_size);
	ffn_inter_buf_2.remalloc(2 * num_tokens * local_ffn_size);
	attention_out.remalloc(num_tokens * hyper_param.hidden_size);
	context_stage_kernel_m_buf.remalloc(local_q_head_num * num_tokens);
	context_stage_kernel_l_buf.remalloc(local_q_head_num * num_tokens);
	...
	for (int64_t i = 0, cur_token_index = 0; i < batch_size; ++i) {
		if (h_is_context_stage[i]) {
			ith_context_req_req_index_cpu[num_context_reqs] = i;
			ith_context_req_token_index_cpu[num_context_reqs] = cur_token_index;
			max_context_req_len = std::max(max_context_req_len, (int64_t)h_input_len[i]);
			num_context_reqs += 1;
			cur_token_index += (int64_t)h_input_len[i];
		} else {
			ith_decoding_req_req_index_cpu[num_decoding_reqs] = i;
			ith_decoding_req_token_index_cpu[num_decoding_reqs] = cur_token_index;
			max_decoding_req_len = std::max(max_decoding_req_len, (int64_t)h_input_len[i]);
			num_decoding_reqs += 1;
			cur_token_index += 1;
		}
	}
	...
	for (int64_t layer_id = 0; layer_id < parallelism_param.local_layer_num; ++layer_id) {
		std::string range_name = "layer " + std::to_string(layer_id);
		...
		st::layer::attention<T>(
			attention_out.ptr,
			// [hongke@1014] we change the layout of k/v cache to [num_layers, num_blocks, local_kv_head_num, block_size, head_dim]
			d_k_cache,
			d_v_cache,
		...
		if (hyper_param.is_gated_ffn) {
			st::layer::gatedFfn<T>(
				...
				cur_layer_weight.ffn_fc1_weight,
				cur_layer_weight.ffn_fc2_weight,
				cur_layer_weight.ffn_fc13_weight,
...
void Gpt<T>::prologue(
	...
	// d_block_table_ = *d_block_table;
	d_k_cache = st::util::convertTensorToRawPtr(k_cache);
    d_v_cache = st::util::convertTensorToRawPtr(v_cache);

	batch_size = input_tokens_batched.size();
	assert (first_token_indexes.size() == static_cast<size_t>(batch_size));

	h_input_lens = new int64_t[batch_size];
    h_is_context_stage = new bool[batch_size];
	h_sum_prev_input_lens = new int64_t[batch_size];
	num_tokens = 0;
...
template<typename T>
std::vector<int64_t> Gpt<T>::pipelinedforward(
	const std::vector<std::vector<int64_t>> &input_tokens_batched,
	const std::vector<int64_t> &first_token_indexes,
	// void* d_k_cache,
	// void* d_v_cache,
	...
	this->prologue(input_tokens_batched, first_token_indexes, k_cache, v_cache, d_block_table);
	for (int64_t layer_id = 0; layer_id < parallelism_param.local_layer_num; ++layer_id) {
		this->ForwardBlock(layer_id);
	}
...
std::vector<int64_t> Gpt<T>::forward(
	...
	// [hongke@1014]
	int32_t* h_input_lens = new int32_t[batch_size];
	...
	d_sum_prev_input_lens.remalloc(batch_size);
	// CUDA_CHECK(cudaMemcpy(d_input_lens.ptr, h_input_lens, sizeof(int64_t) * batch_size, cudaMemcpyHostToDevice));
	CUDA_CHECK(cudaMemcpy(d_input_lens.ptr, h_input_lens, sizeof(int32_t) * batch_size, cudaMemcpyHostToDevice));
	CUDA_CHECK(cudaMemcpy(d_sum_prev_input_lens.ptr, h_sum_prev_input_lens, sizeof(int64_t)*batch_size, cudaMemcpyHostToDevice));
	sync_check_cuda_error();
	...
	h_input_lens = nullptr;
	h_is_context_stage = nullptr;
	h_sum_prev_input_lens = nullptr;
	nvtxRangePop();
    return output_tokens;
}

template<typename T>
std::vector<int64_t> Gpt<T>::epilogue() {
	std::vector<int64_t> output_tokens(batch_size);
