...
public: 
	...
	virtual std::vector<torch::Tensor>  GetFlattenWeight() = 0;
	virtual void initDummyWeightFSDP() = 0;
	virtual void LazyInitWeight(bool enable_ipc_mem, bool is_alloc) = 0;
	...
	virtual std::vector<int64_t> pipelinedforward(
		// input data && metadata
		const std::vector<std::vector<int64_t>> &input_tokens_batched,
		const std::vector<int64_t> &first_token_indexes,
		// key-value management
	...
	virtual std::vector<int64_t> epilogue() = 0;

	virtual void ForwardBlock(int64_t layer_id) = 0;

	virtual void init_communicator(const ncclUniqueId& tp_id, const ncclUniqueId& pp_id) = 0;

	virtual void init_p2p_communicator(const ncclUniqueId& peer_id) = 0;

	virtual void send_weight(int64_t layer_id) =0;

	virtual void recv_weight(int64_t layer_id) =0;

	virtual void wait_stream() =0;

	virtual	void nccl_group_start() =0;

	virtual void nccl_group_end() =0;

	virtual torch::Tensor GetCosSinCache() = 0;
