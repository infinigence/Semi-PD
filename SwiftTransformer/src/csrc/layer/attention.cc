...
#include "kernel/flash_attn/flash_attn.h"
...
void attention(
	...
	const int32_t* input_len,				// [batch_size], gpu. The length of the i-th input. For context stage it is the number of tokens in user's request, while for decoder stage it is the number of previous tokens 
	const bool* is_context_stage_cpu,	// [batch_size]. Whether the i-th input is in context stage or decoder stage
	const int32_t* block_table,				// [batch_size, max_num_block_per_req], gpu. The block table of the i-th input. For context stage it is the block table of user's request, while for decoder stage it is the block table of previous tokens
	...
	if (perform_rotary_embedding) {
		...
		kernel::rotaryPosiEmbeddingBatchedWithCosSinCache(
			qkv_buf,
			d_token_indexes,
			(half*)cos_sin_cache,
			num_tokens,
	...
	if (num_context_reqs != 0) {
		...
		// 	);
		// 	sync_check_cuda_error();
		// } else if (local_q_head_num > local_kv_head_num && std::is_same_v<T, half>) {
		if (std::is_same_v<T, half>) {
	...
	if (num_decoding_reqs != 0) {
		...
		// [hongke@1014] change KV cache layout to [num_layers, num_blocks, local_kv_head_num, block_size, head_dim]
		kernel::FlashDecoding<T>(
			attn_out_buf, 
			qkv_buf,
			k_cache,
			v_cache,
...
template void attention( \
	...
	const T* input, const int32_t* input_len, const bool* is_context_stage_cpu, const int32_t* block_table, const int64_t block_num_per_layer, const int64_t* d_token_indexes, \
	...
	const int64_t batch_size, const int64_t num_tokens, const int64_t hidden_size, const int64_t num_layers, const int64_t num_q_heads, const int64_t num_kv_heads, const int64_t head_dim, const bool perform_rotary_embedding, const bool perform_rotary_embedding_scaling, const int64_t layer_id, \
