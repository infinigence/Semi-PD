...
NcclComm stNcclInitWithConfig(int64_t world_size, int64_t rank, const ncclUniqueId& nccl_id, cudaStream_t stream, bool real_init)
{
    NcclComm nccl_comm;
    nccl_comm.rank = rank;
    nccl_comm.size = world_size;
    ...
    NCCL_CHECK(ncclCommInitRankConfig(&nccl_comm.comm, nccl_comm.size, nccl_id, nccl_comm.rank, &config));
    return nccl_comm;
}
